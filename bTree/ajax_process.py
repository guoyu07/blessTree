...
def ajax_1(request):
    ...
    if user_id and user_name and tree_name:
        try:
            ...
            user.save()
            msg = Tree(owner=user.friends, tree_name=user.friends.tree_name, type=4, action_time=time.time(), read=True,
                       source_id=user_id, content=u'创建了祝福树')
            msg.save()
...
def ajax_2(request):
    ...
    if user_id and load_begin:
        ...
        # # 注意成功不反悔ret1，省去处理的麻烦
        name_dict = {"twz": "hello", "zqxt": "I am teaching Django"}
...
def ajax_9(request):
    ...
    if user_id and source_id and bless_con:
        user = User.objects.get(openid=user_id)
        will = Tree(owner=user, tree_name=user.tree_name, type=5, action_time=time.time(), read=True, source_id=source_id,
                    content=bless_con)
        user.willing = 'yes'
        will.save()
        user.save()
